  ...
  public Vec<String> process(String[] args, ArgFn longArg, ArgFn shortArg) {
    ...
    DoFn doArg = (c, i0, i1, f) -> {
      if (i0==-1 || i0==c.length()) {
        ...
        f.run(arg, i.get(), () -> {
          if (taken.get()) error(arg+": Attempting to consume multiple arguments");
          taken.set(true);
          return val;
        });
        if (!taken.get()) error(arg+": Unexpected argument");
      }
    };
    ...
    while (i.get() < args.length) {
      ...
      if (c.startsWith("--")) {
        if (c.equals("--")) break;
        int s = c.indexOf("=");
        if (longArg==null) error("Long arguments not supported");
        doArg.run(c, s, s+1, longArg);
      } else if (c.startsWith("-")) {
        int s = c.indexOf("=");
        if (shortArg==null) error("Short arguments not supported");
